...
class _TaskManagementPageState extends State<TaskManagementPage> {
  ...
  // 🔥 STATE BARU UNTUK MENGHILANGKAN FREEZE & SCROLL RESET
  List<GroupedTask> _groupedTasks = [];
  bool _isLoadingUI = true;
  bool _isSyncing = false; // Pelindung agar tidak spam refresh ke server
  ...
  void initState() {
    ...
    // 1. Proses data lokal pertama kali (Sangat Cepat)
    _processTasksInBackground();

    // 2. Dengarkan perubahan di Hive (Tanpa ValueListenableBuilder yang berat)
    _taskBox.listenable().addListener(_onHiveChanged);

    // 3. Sinkronisasi Server
    ...
    _connectivitySubscription = Connectivity().onConnectivityChanged.listen((result) {
      ...
      // Hanya sync jika sedang online DAN tidak sedang dalam proses sync
      if (isOnline && !_isSyncing) {
        debugPrint("🌐 Jaringan kembali ONLINE! Memulai sinkronisasi otomatis...");
        _syncDataFromServer();
      }
    });
  }
  ...
  // ===========================================================================
  // UI & STATE PROCESSING (BEBAS FREEZE)
  // ===========================================================================
  void _onHiveChanged() {
    _processTasksInBackground();
  }

  Future<void> _processTasksInBackground() async {
    final rawTasks = _myRawTasks;
    // Lakukan grouping di background (tidak menahan build UI)
    final newGroupedTasks = await dapatkanTugasTerkelompok(rawTasks);
    
    if (mounted) {
      setState(() {
        _groupedTasks = newGroupedTasks;
        _isLoadingUI = false;
      });
    }
  }
  ...
  // ===========================================================================
  // LOGIKA DATABASE & MONGODB (SAMA SEPERTI SEBELUMNYA)
  // ===========================================================================
  ...
  Future<String> _resolveNamaKelas(String idKelasHex) async {
    ...
    if (_kelasCacheNames.containsKey(cleanId)) return _kelasCacheNames[cleanId]!;
    ...
    try {
      ...
      final kelasDoc = await MongoDatabase.kelasCollection.findOne(where.id(ObjectId.parse(cleanId)));
  ...
  Set<String> get _allMataKuliah {
    ...
    for (final task in _taskBox.values) {
      if (_extractId(task.idUser) == cleanUserId && task.namaMkSnapshot != null) {
  ...
  Future<void> _syncDataFromServer() async {
    if (_isSyncing) return; // Mencegah server dipanggil berulang-ulang
    
    try {
      final user = context.read<LoginViewModel>().user;
      if (user == null) return;

      setState(() => _isSyncing = true); // Kunci gembok sync
      ...
      for (var data in rawCloudTasks) {
        try {
          if (data['_id'] != null) data['_id'] = _extractId(data['_id'].toString());
          if (data['id_user'] != null) data['id_user'] = _extractId(data['id_user'].toString());

          final t = TaskModel.fromMongo(data);
          t.isSynced = true;
          cloudTasks.add(t);
        } catch (e) {
          debugPrint("⚠️ Error parsing TaskModel: $e");
        }
      }
      ...
      for (var key in localKeys) {
        ...
        if (taskInHive != null && taskInHive.isSynced && !cloudIds.contains(_extractId(key))) {
          keysToDelete.add(key);
        }
      }

      if (rawCloudTasks.isNotEmpty && cloudTasks.isEmpty) {
        debugPrint("⚠️ Parsing data gagal. Mencegah penghapusan.");
      } else {
      ...
      // (Data akan otomatis tampil berkat listener Hive)
    } catch (e) {
      debugPrint("❌ Gagal Sinkronisasi: $e");
    } finally {
      if (mounted) setState(() => _isSyncing = false); // Buka gembok sync
    }
  }
  ...
  Future<void> _deleteGroupedTask(GroupedTask groupedTask) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Apakah Anda yakin ingin menghapus tugas ini untuk seluruh kelas yang bersangkutan ($daftarKelasText)?'),
        actions: [
            ...
            child: RefreshIndicator(
              ...
              child: _isLoadingUI
                  ...
                  : _groupedTasks.isEmpty
                      ...
                      // 🔥 KINI MENGGUNAKAN LIST VIEW BIASA (Bisa discroll lancar!)
                      : ListView.builder(
                          physics: const AlwaysScrollableScrollPhysics(),
                          padding: const EdgeInsets.all(16),
                          itemCount: _groupedTasks.length,
                          itemBuilder: (context, index) {
                            return _buildGroupedTaskCard(_groupedTasks[index]);
                          },
                        ),
            ),
          ),
            ...
            Padding(
              ...
              child: FilterChip(
                ...
                onSelected: (_) {
                  ...
                  _processTasksInBackground(); // Panggil ulang proses filter
                },
                backgroundColor: Colors.white,
                selectedColor: secondaryBlue,
                showCheckmark: false,
                side: BorderSide(color: _selectedFilterMatkul.isEmpty ? secondaryBlue : Colors.grey.shade300),
              ),
            ),
            ...displayMatkul.map((matkul) {
              ...
              return Padding(
                ...
                child: FilterChip(
                  label: Text(
                    matkul,
                    style: TextStyle(fontSize: 13, color: isSelected ? Colors.white : secondaryBlue),
                  ),
                  selected: isSelected,
                  onSelected: (_) {
                    setState(() {
                      isSelected ? _selectedFilterMatkul.remove(matkul) : _selectedFilterMatkul.add(matkul);
                    });
                    _processTasksInBackground(); // Panggil ulang proses filter
                  },
                  backgroundColor: Colors.white,
                  selectedColor: secondaryBlue,
                  showCheckmark: false,
                  side: BorderSide(color: isSelected ? secondaryBlue : Colors.grey.shade300),
                ),
              );
            }),
          ],
              ...
              children: [
                    ...
                    children: [
                      Expanded(
                        child: Text(
                          ...
                          style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: secondaryBlue, height: 1.3),
                          softWrap: true,
                        ),
                      ),
                      ...
                      Expanded(
                        child: RichText(
                          text: TextSpan(
                            ...
                            style: const TextStyle(fontSize: 13, color: primaryBlue, fontWeight: FontWeight.w600),
                            children: [
                              TextSpan(
                ...
                Row(
                  children: [
                    ...
                    Text('Deadline: ${_formatDateTime(groupedTask.deadline)}', style: TextStyle(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.w500)),
                    if (representatifTask.lampiran != null && representatifTask.lampiran!.isNotEmpty) ...[
                      const SizedBox(width: 12),
                      const Text('•', style: TextStyle(color: Colors.grey)),
                      const SizedBox(width: 12),
                      const Icon(Icons.attach_file_rounded, size: 15, color: Colors.grey),
                      const SizedBox(width: 4),
                      Text('${representatifTask.lampiran!.length}', style: TextStyle(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.w500)),
                    ],
                  ],
                ),
                const Padding(
                ...
                Row(
                  ...
                  children: [
                    Row(
                      children: [
                        if (_isSyncing && !groupedTask.isSynced)
                          const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: accentOrange))
                        else
                          Icon(
                            groupedTask.isSynced ? Icons.cloud_done_rounded : Icons.cloud_upload_rounded,
                            size: 16,
                            color: groupedTask.isSynced ? Colors.green.shade600 : accentOrange,
                          ),
                        const SizedBox(width: 6),
                        Text(
                          groupedTask.isSynced ? "Tersinkronisasi" : "Menunggu Jaringan",
                          style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: groupedTask.isSynced ? Colors.green.shade700 : accentOrange),
                        ),
                      ],
                    ),
                    Row(
                      children: [
                        InkWell(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: secondaryBlue.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                            child: const Row(children: [Icon(Icons.edit_rounded, size: 14, color: secondaryBlue), SizedBox(width: 4), Text('Edit', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: secondaryBlue))]),
                          ),
                        ),
                        const SizedBox(width: 8),
                        InkWell(
                          ...
                          child: Container(
                            ...
                            child: const Row(children: [Icon(Icons.delete_outline_rounded, size: 14, color: Colors.red), SizedBox(width: 4), Text('Hapus', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.red))]),
                          ),
                        ),
                      ],
                    ),
  ...
  void _showTaskDetailBottomSheet(BuildContext context, GroupedTask groupedTask) {
    final representatifTask = groupedTask.originalTasks.first;
    final String daftarKelasText = groupedTask.targetKelasList.join(', ');
      ...
      builder: (context) {
        return Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Center(child: Container(margin: const EdgeInsets.only(top: 12, bottom: 16), width: 40, height: 4, decoration: BoxDecoration(color: Colors.grey.shade300, borderRadius: BorderRadius.circular(10)))),
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 24),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text("Detail Tugas", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: primaryBlue)),
                  ...
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        padding: const EdgeInsets.all(16),
                        decoration: BoxDecoration(color: secondaryBlue.withOpacity(0.08), borderRadius: BorderRadius.circular(12), border: Border.all(color: secondaryBlue.withOpacity(0.15))),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text("Nama Tugas", style: TextStyle(fontSize: 12, color: secondaryBlue, fontWeight: FontWeight.w600)),
                            const SizedBox(height: 6),
                            Text(groupedTask.namaTugas, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: secondaryBlue, height: 1.3)),
                          ],
                        ),
                      ),
                      const SizedBox(height: 24),
                      _buildDetailRow(Icons.book, "Mata Kuliah", groupedTask.matkulNamaSaja),
                      const SizedBox(height: 16),
                      _buildDetailRow(Icons.group, "Target Kelas", daftarKelasText.isEmpty ? "Semua Kelas" : daftarKelasText),
                      const SizedBox(height: 16),
                      _buildDetailRow(Icons.access_time_filled, "Deadline", _formatDateTime(groupedTask.deadline), iconColor: accentOrange),
                      const SizedBox(height: 24),
                      const Text("Deskripsi", style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: primaryBlue)),
                      const SizedBox(height: 8),
                      Container(
                        width: double.infinity,
                        padding: const EdgeInsets.all(16),
                        ...
                        child: Text(groupedTask.deskripsi ?? "Tidak ada deskripsi tambahan.", style: TextStyle(color: Colors.grey.shade700, height: 1.5)),
                      ),
                      if (representatifTask.lampiran != null && representatifTask.lampiran!.isNotEmpty) ...[
                        const SizedBox(height: 24),
                        const Text("Lampiran", style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: primaryBlue)),
                        const SizedBox(height: 8),
                        ...representatifTask.lampiran!.map((lamp) {
                          final fileName = lamp['title'] ?? lamp['name'] ?? 'Lampiran';
                          return ListTile(
                            contentPadding: EdgeInsets.zero,
                            onTap: () async {
                              String dataBase64 = lamp['data']?.toString() ?? '';
                              String uriString = lamp['uri']?.toString() ?? '';

                              if (dataBase64.isNotEmpty && dataBase64 != 'null') {
                                try {
                                  if (!context.mounted) return;
                                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Mengunduh & Membuka file dari cloud..."), duration: Duration(seconds: 1)));
                                  ...
                                  if (result.type != ResultType.done) {
                                    if (!context.mounted) return;
                                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Gagal membuka file: ${result.message}")));
                                  }
                                  return;
                                } catch (e) {
                                  if (!context.mounted) return;
                                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("⚠️ File rusak atau tidak valid.", style: TextStyle(color: Colors.white)), backgroundColor: Colors.red));
                                }
                              } else if (uriString.isNotEmpty && uriString != 'null') {
                                ...
                                if (await file.exists()) {
                                  ...
                                  if (!context.mounted) return;
                                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("⚠️ File draf lokal tidak ditemukan (cache HP dibersihkan)."), backgroundColor: Colors.red));
                                }
                              } else {
                                if (!context.mounted) return;
                                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("⚠️ Data lampiran kosong."), backgroundColor: Colors.red));
                              }
                            },
                            leading: Container(
                              padding: const EdgeInsets.all(8),
                              decoration: BoxDecoration(color: secondaryBlue.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
                              child: Icon(lamp['type'] == 'file' ? Icons.insert_drive_file : Icons.link, color: secondaryBlue, size: 20),
                            ),
                            title: Text(fileName, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                            subtitle: Text(lamp['type'] == 'file' ? 'Ketuk untuk melihat file' : 'Tautan Web', style: const TextStyle(fontSize: 12)),
                          );
                        }),
                      ],
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(value, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: primaryBlue)),
            ],
          ),
        ),
      ],
