...
@section('page_title', 'Dashboard Tim Penjadwalan')
    ...
    <div>
        ...
        @if(session('conflict_detail'))
            <p class="text-xs mt-1 text-red-600 font-mono bg-red-100 px-2 py-1 rounded inline-block">
                ...
                ({{ session('conflict_detail')->hari }}, {{ session('conflict_detail')->jam_mulai }}–{{ session('conflict_detail')->jam_selesai }})
            </p>
        @endif
    </div>
</div>
...
<div class="bg-gradient-to-r from-indigo-900 to-indigo-700 rounded-2xl p-6 mb-8 text-white shadow-lg relative overflow-hidden">
    ...
    <div class="relative z-10 mt-4 flex gap-3">
        <a href="{{ route('penjadwalan.schedules.create') }}" class="bg-yellow-400 text-indigo-900 px-4 py-2 rounded-lg text-sm font-bold shadow hover:bg-yellow-300 transition flex items-center gap-2">
            <i class="fas fa-plus"></i> Input Jadwal Baru
        </a>
        <a href="{{ route('penjadwalan.requests.index') }}" class="bg-white/10 backdrop-blur text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-white/20 transition flex items-center gap-2 border border-white/20">
            <i class="fas fa-inbox"></i> Kelola Request
            @if($pending_requests > 0)
                <span class="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{{ $pending_requests }}</span>
            @endif
        </a>
    </div>
</div>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 border-l-4 border-l-slate-400">
        ...
        <h3 class="text-3xl font-black text-slate-700">{{ $count_draft }}</h3>
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 border-l-4 border-l-yellow-400">
        ...
        <h3 class="text-3xl font-black text-slate-800">{{ $count_final }}</h3>
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 border-l-4 border-l-emerald-500">
        ...
        <h3 class="text-3xl font-black text-slate-800">{{ $count_published }}</h3>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 mb-8">
    <div class="flex justify-between items-center mb-3">
        ...
        <span class="text-xs text-slate-500">{{ round(($count_published / $total) * 100) }}% Published</span>
    </div>
    <div class="h-3 bg-slate-100 rounded-full overflow-hidden flex gap-0.5">
        @if($count_draft > 0)
        <div class="bg-slate-400 h-full rounded-l-full transition-all duration-500" style="width: {{ ($count_draft / $total) * 100 }}%"></div>
        @endif
        @if($count_final > 0)
        <div class="bg-yellow-400 h-full transition-all duration-500" style="width: {{ ($count_final / $total) * 100 }}%"></div>
        @endif
        @if($count_published > 0)
        <div class="bg-emerald-500 h-full rounded-r-full transition-all duration-500" style="width: {{ ($count_published / $total) * 100 }}%"></div>
        @endif
    </div>
...
<div class="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-8 shadow-sm">
    <div class="flex justify-between items-center">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h4 class="font-bold text-amber-900 text-sm">{{ $pending_requests }} Request Perubahan Jadwal Menunggu</h4>
                <p class="text-xs text-amber-700">Dosen telah mengajukan perubahan jadwal yang perlu divalidasi.</p>
            </div>
        </div>
        <a href="{{ route('penjadwalan.requests.index') }}" class="bg-amber-500 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-amber-600 transition shadow-sm flex-shrink-0">
            Kelola Sekarang →
        </a>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
    <div class="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
        ...
        <a href="{{ route('penjadwalan.schedules.index') }}" class="text-indigo-600 text-sm font-semibold hover:text-indigo-800">
            Lihat Semua →
        </a>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($schedules->take(5) as $jadwal)
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-6 py-4">
                        <p class="font-medium text-slate-800">{{ $jadwal->hari }}, {{ \Carbon\Carbon::parse($jadwal->jam_mulai)->format('H:i') }}–{{ \Carbon\Carbon::parse($jadwal->jam_selesai)->format('H:i') }}</p>
                    ...
                    <td class="px-6 py-4 text-right">
                        <a href="{{ route('penjadwalan.schedules.edit', $jadwal->id) }}" class="text-indigo-600 hover:text-indigo-800 text-xs font-semibold bg-indigo-50 px-3 py-1.5 rounded border border-indigo-100">
