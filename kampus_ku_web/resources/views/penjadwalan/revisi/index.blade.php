...
@section('page_title', 'Periode Revisi Jadwal')
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 border-l-4 border-l-emerald-500">
        ...
        <h3 class="text-3xl font-black text-slate-800">{{ $count_aktif }}</h3>
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 border-l-4 border-l-indigo-500">
        ...
        <h3 class="text-3xl font-black text-slate-800">{{ $count_semester }}</h3>
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 border-l-4 border-l-purple-500">
        ...
        <h3 class="text-3xl font-black text-slate-800">{{ $count_matkul }}</h3>
        ...
        <table class="w-full text-left text-sm min-w-[800px]">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-6 py-4">
                        @if($periode->scope == 'SEMESTER')
                            <p class="text-xs text-slate-500 italic">Semua dosen</p>
                        @else
                            <p class="text-xs font-semibold text-slate-700">{{ $periode->nama_jadwal ?? '-' }}</p>
                            <p class="text-[10px] text-slate-400">{{ $periode->nama_dosen ?? '-' }}</p>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex gap-2 justify-end">
                            ...
                            <form action="{{ route('penjadwalan.revisi.toggle', $periode->id) }}" method="POST">
                                @csrf @method('PATCH')
                                <button type="submit"
                                        onclick="return confirm('{{ $periode->is_active ? 'Nonaktifkan' : 'Aktifkan' }} periode ini?')"
                                        class="text-xs font-semibold px-3 py-1.5 rounded border transition
                                               {{ $periode->is_active
                                                   ? 'text-amber-600 bg-amber-50 border-amber-200 hover:bg-amber-100'
                                                   : 'text-emerald-600 bg-emerald-50 border-emerald-200 hover:bg-emerald-100' }}">
                                    <i class="fas {{ $periode->is_active ? 'fa-pause' : 'fa-play' }} mr-1"></i>
                                    {{ $periode->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                                </button>
                            </form>

                            {{-- Hapus --}}
                            <form action="{{ route('penjadwalan.revisi.destroy', $periode->id) }}" method="POST"
...
<div id="modal-create" class="fixed inset-0 bg-slate-900/50 hidden z-50 flex items-center justify-center backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl max-w-lg w-full m-4 overflow-hidden">
        <form action="{{ route('penjadwalan.revisi.store') }}" method="POST">
            @csrf
            <div class="p-6">
                ...
                <div class="space-y-4">
                    ...
                    {{-- Pilih Jadwal (muncul jika scope MATKUL) --}}
                        ...
                        <select name="id_jadwal"
                            ...
                            @foreach($schedules as $jadwal)
                                <option value="{{ $jadwal->id }}">
                                    {{ $jadwal->nama_mk }} — {{ $jadwal->nama_dosen }}
                                </option>
                            @endforeach
                        </select>
                    </div>
