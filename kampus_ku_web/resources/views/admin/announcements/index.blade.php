...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 border-l-4 border-l-emerald-500">
        ...
        <h3 class="text-3xl font-black text-slate-800">{{ $total_bulan_ini ?? 0 }}</h3>
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 border-l-4 border-l-amber-400">
        ...
        <h3 class="text-3xl font-black text-slate-800">{{ $total_dibaca ?? 0 }}</h3>
        ...
        <table class="w-full text-left text-sm min-w-[700px]">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    <td class="px-6 py-4">
                        <p class="font-bold text-slate-800">{{ $ann->judul }}</p>
                        <span class="bg-indigo-50 text-indigo-600 text-[10px] font-bold px-2 py-0.5 rounded border border-indigo-100 mt-1 inline-block">
                            {{ is_array($ann->kategori) ? implode(', ', $ann->kategori) : ($ann->kategori ?? 'Umum') }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        @if($ann->id_prodi)
                    ...
                    <td class="px-6 py-4">
                        <span class="text-sm font-bold text-slate-700">{{ $total_baca }}</span>
                    ...
                    <td class="px-6 py-4">
                        ...
                        <p class="text-[10px] text-slate-400">
                            {{ \Carbon\Carbon::parse($ann->created_at)->format('H:i') }} WIB
                        </p>
                    </td>
