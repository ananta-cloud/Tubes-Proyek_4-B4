...
<head>
    ...
    <title>SIGMA - <?php echo $__env->yieldContent('page_title', 'Dashboard'); ?></title>
...
<body class="bg-slate-50 flex h-screen font-sans text-slate-800 overflow-hidden">
    ...
    <aside class="w-64 bg-indigo-900 text-white flex flex-col shadow-xl z-20">
            ...
            <div>
                ...
                <p class="text-[10px] text-indigo-300 uppercase tracking-widest">
                    <?php if(auth()->user()->role == 'MANAJEMEN'): ?>
                        Portal Universitas
                    <?php elseif(auth()->user()->role == 'TIM_PENJADWALAN'): ?>
                        Tim Penjadwalan
                    <?php else: ?>
                        Portal Jurusan
                    <?php endif; ?>
        ...
        <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
            ...
            <a href="<?php echo e(route('penjadwalan.dashboard')); ?>"
               class="w-full flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition
                      <?php echo e(request()->routeIs('penjadwalan.dashboard') ? 'bg-indigo-800 text-white' : 'text-indigo-200 hover:bg-indigo-800'); ?>">
                <i class="fas fa-tachometer-alt w-5 text-center"></i> Dashboard
            </a>

            <a href="<?php echo e(route('penjadwalan.schedules.index')); ?>"
               class="w-full flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition
                      <?php echo e(request()->routeIs('penjadwalan.schedules.*') ? 'bg-indigo-800 text-white' : 'text-indigo-200 hover:bg-indigo-800'); ?>">
                <i class="fas fa-calendar-alt w-5 text-center"></i> Kelola Jadwal
            </a>

            <a href="<?php echo e(route('penjadwalan.revisi.index')); ?>"
            class="w-full flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition
                    <?php echo e(request()->routeIs('penjadwalan.revisi.*') ? 'bg-indigo-800 text-white' : 'text-indigo-200 hover:bg-indigo-800'); ?>">
                <i class="fas fa-calendar-check w-5 text-center"></i> Periode Revisi
            </a>
            
            <a href="<?php echo e(route('penjadwalan.requests.index')); ?>"
               class="w-full flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition
                      <?php echo e(request()->routeIs('penjadwalan.requests.*') ? 'bg-indigo-800 text-white' : 'text-indigo-200 hover:bg-indigo-800'); ?>">
            ...
            <a href="<?php echo e(url('/jurusan/schedules')); ?>"
               class="w-full flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition
                      <?php echo e(request()->is('jurusan/schedules*') ? 'bg-indigo-800 text-white' : 'text-indigo-200 hover:bg-indigo-800'); ?>">
                <i class="fas fa-calendar-alt w-5 text-center"></i> Kelola Jadwal
            </a>

            <a href="<?php echo e(url('/jurusan/announcements')); ?>"
               class="w-full flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition
                      <?php echo e(request()->is('jurusan/announcements*') ? 'bg-indigo-800 text-white' : 'text-indigo-200 hover:bg-indigo-800'); ?>">
                <i class="fas fa-bullhorn w-5 text-center"></i> Pengumuman
            </a>

            <a href="<?php echo e(url('/jurusan/master-matkul')); ?>"
               class="w-full flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition
                      <?php echo e(request()->is('jurusan/master-matkul*') ? 'bg-indigo-800 text-white' : 'text-indigo-200 hover:bg-indigo-800'); ?>">
            ...
            <a href="<?php echo e(route('manajemen.dashboard')); ?>"
               class="w-full flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition
                      <?php echo e(request()->routeIs('manajemen.dashboard') ? 'bg-indigo-800 text-white' : 'text-indigo-200 hover:bg-indigo-800'); ?>">
                <i class="fas fa-bullhorn w-5 text-center"></i> Pengumuman
        ...
        <div class="p-4 border-t border-indigo-800">
            <div class="flex items-center gap-3 bg-indigo-950 p-3 rounded-lg border border-indigo-800">
                <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode(auth()->user()->nama)); ?>&background=eab308&color=1e3a8a"
                     class="w-10 h-10 rounded-full border-2 border-yellow-400">
                <div class="overflow-hidden">
                    <p class="text-sm font-bold truncate"><?php echo e(auth()->user()->nama); ?></p>
                    <p class="text-xs text-indigo-300 truncate"><?php echo e(auth()->user()->role); ?></p>
                </div>
            </div>
        </div>
    </aside>
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header class="bg-white px-6 py-4 shadow-sm flex flex-col md:flex-row justify-between items-center z-10">
            <div>
                <h2 class="font-bold text-xl text-slate-800"><?php echo $__env->yieldContent('page_title', 'Dashboard SIGMA'); ?></h2>
...
<?php /**PATH D:\Semester 4\Proyek 4\Tubes-Proyek_4-B4\kampus_ku_web\resources\views/layouts/app.blade.php ENDPATH**/ ?>