...
import 'package:kampus_ku_mobile/data/models/user_model.dart';
...
import 'package:kampus_ku_mobile/features/dosen/requests/request_form_page.dart';
import 'package:kampus_ku_mobile/features/dosen/requests/my_requests_page.dart';

class DosenMainPage extends StatefulWidget {
  final UserModel user;
  const DosenMainPage({super.key, required this.user});

  @override
  State<DosenMainPage> createState() => DosenMainPageState();
}

class DosenMainPageState extends State<DosenMainPage> {
  int currentIndex = 0;
  ...
  Widget build(BuildContext context) {
    ...
    final requestController = context.watch<DosenRequestController>();
    ...
    return Scaffold(
            ...
            Expanded(
              ...
              child: IndexedStack(
                ...
                children: [
                  ...
                  _requestTab(requestController), // 3
      ...
      bottomNavigationBar: _bottomNav(requestController),
    );
  }

  // ================= HEADER =================
  Widget _header(DosenRequestController requestController) {
    final pendingCount = requestController.myRequests
        .where((r) => r.isPending)
        .length;
    return Container(
  ...
  Widget _dashboardDosen(
    ...
    DosenRequestController requestController,
  ) {
    ...
    var todaySchedules = sController.schedules
        .where((s) => s.hari.toLowerCase() == todayStr)
        .toList();
            ...
            return GestureDetector(
              onTap: () {
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        AnnouncementDetailPage(announcement: ann),
                  ),
                );
              },
  ...
  Widget _requestTab(DosenRequestController requestController) {
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: TabBarView(
              children: [
                RequestFormPage(user: widget.user),
                MyRequestsPage(user: widget.user),
              ],
            ),
          ),
        ],
  ...
  // ================= NAVBAR 5 MENU =================
  Widget _bottomNav(DosenRequestController requestController) {
    final pendingCount = requestController.myRequests
        .where((r) => r.isPending)
        .length;
    return Container(
